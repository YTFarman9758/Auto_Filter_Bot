...
from dreamxbotz.gate import wrap_with_gate
import time
    ...
    try:
        ...
        if message.command[1].startswith("reff_"):
            ...
            if fromuse >= 100:
                ...
                ok = await add_premium(client, user_id, "1 month")

                if ok:
                    await message.reply_text("🎉 Congratulations! You earned 1 Month Premium.")
                    await client.send_message(
                        user_id,
                        "🎉 You received 1 Month Premium by Referral."
                    )

                    for admin in ADMINS:
                        await client.send_message(
                            admin,
                            f"✅ {uss.mention} ({user_id}) received Premium via Referral."
                        )
            else:
